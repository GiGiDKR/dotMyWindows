menu(where=sel.count>0 type='file|dir|drive|namespace|back' mode="multiple" title='Gestion fichiers' image=\uE253)
{
	menu(separator="after" title=title.copy_path image=icon.copy_path)
	{
		...
		separator
		item(mode="single" title=@path.wsl(sel.path) cmd=command.copy(path.wsl(sel.path)))
		separator
	...
	item(mode="single" type="file" title="Modifier l'extension" image=\uE0B5 cmd=if(input("Modifier l'extension", "Saisir l'extension"), 
		io.rename(sel.path, path.join(sel.dir, sel.file.title + "." + input.result))))
	separator
	menu(separator="after" image=\uE290 title=title.select)
	{
		item(title="Tous" image=icon.select_all cmd=command.select_all)
		item(title="Autres" image=icon.invert_selection cmd=command.invert_selection)
		item(title="Aucun" image=icon.select_none cmd=command.select_none)
	}
	...
	menu(type='file|dir|back.dir' mode="single" title='Attributs')
	{
